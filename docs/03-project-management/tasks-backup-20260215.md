...
| Phase 7 | 62.5% (5/8) | 🚧 진행 중 (프로그래머스 통합) |
...
**Phase 7 하이라이트**:
- ✅ Puppeteer 기반 검색 (Task 7.2 완료)
- ✅ cheerio 기반 문제 상세 (Task 7.3 완료, 1-2초 응답)
- ✅ get_programmers_problem 도구 (Task 7.5 완료)
- ✅ URL/숫자 입력 지원 (parseProgrammersUrl)
- 📋 테스트 코드 작성 (Task 7.7 진행 예정)
- 📋 문서 업데이트 (Task 7.8 진행 중)
...
**구현 완료 내역**:
- ✅ getProblem() 메서드 구현 (`src/api/programmers-scraper.ts`)
- ✅ fetch + cheerio 기반 파싱 (BOJScraper 패턴 재사용)
- ✅ 에러 처리 (NOT_FOUND, TIMEOUT, NETWORK_ERROR, PARSE_ERROR)
- ✅ Rate Limiting 통합 (초당 1회)
- ✅ LRU 캐싱 통합 (TTL 30일)
- ✅ HTML 파서 구현 (`src/utils/html-parser.ts`)
- ✅ URL 파서 구현 (`src/utils/url-parser.ts`)

**산출물**:
- `src/api/programmers-scraper.ts`: getProblem() 메서드 추가 (122줄)
- `src/utils/html-parser.ts`: parseProgrammersProblemContent() 추가 (99줄)
- `src/utils/url-parser.ts`: parseProgrammersUrl() 추가 (신규 파일)
- `tests/api/programmers-scraper.test.ts`: 단위 테스트 9개
- `tests/utils/html-parser.test.ts`: 단위 테스트 14개
...
**구현 완료 내역**:
- ✅ `get_programmers_problem` 도구 구현
  - 파일: `src/tools/get-programmers-problem.ts` (194줄)
  - Zod 스키마 정의 (URL 또는 숫자 지원)
  - ProgrammersScraper.getProblem() 호출
  - 마크다운 포맷팅 (제목, 레벨, 설명, 제한사항, 예제)
  - 에러 처리 (NOT_FOUND, TIMEOUT, PARSE_ERROR)
- ✅ MCP 서버에 도구 등록 (`src/index.ts`)
- ✅ 단위 테스트 작성 (`tests/tools/get-programmers-problem.test.ts`, 9개)

**산출물**:
- `src/tools/get-programmers-problem.ts`: 194줄
- `tests/tools/get-programmers-problem.test.ts`: 9개 테스트 통과
- `src/index.ts`: MCP 도구 등록 추가

**인수 조건 달성**:
- ✅ `get_programmers_problem` 도구 구현 완료
- ✅ Zod 스키마 정의 (union 타입, URL 지원)
- ✅ 단위 테스트 9개 통과
- ✅ 마크다운 출력 검증 완료

**참고**:
- `search_programmers_problems` 도구는 Task 7.2에서 이미 구현 완료
